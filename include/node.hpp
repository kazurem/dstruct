...
class SNode : public BaseNode<T> {
  public:
    SNode(T val = T(), SNode<T> *nptr = nullptr)
        : BaseNode<T>(val), _nptr(nptr) { }
...
class DNode : public BaseNode<T> {
  public:
    DNode(T val = T(), DNode<T> *pptr = nullptr, DNode<T> *nptr = nullptr)
        : BaseNode<T>(val), _pptr(pptr), _nptr(nptr) { }
