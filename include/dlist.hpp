...
T DLList<T>::end() const {
    if (!head())
        throw std::underflow_error("Can't get end value. List is empty!");
    DNode<T> *tptr = head();
    while (tptr->nptr())
        tptr = tptr->nptr();
    return tptr->val();
...
void DLList<T>::insert(int idx, T val) {
    if (idx < 0 || idx > size())
        throw std::out_of_range("Index range out of range");
...
template <typename T>
DNode<T> *DLList<T>::head() const {
    return _head;
}
template <typename T>
void DLList<T>::head(DNode<T> *ptr) {
    _head = ptr;
}
